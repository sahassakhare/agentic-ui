...
import {
  evaluateDecision, type DecisionField, type DecisionRule, type DecisionOp, type HitPolicy, type DecisionTable,
} from '../decision/decision-eval';

const OPS: DecisionOp[] = ['any', '==', '!=', '>', '<', '>=', '<=', 'in'];
...
@Component({
        ...
        <section class="card">
          <div class="cols">
            <div class="colset">
              ...
              @for (f of inputs(); track $index) { <span class="chip in"><input [ngModel]="f.name" (ngModelChange)="rename('inputs', $index, $event)" /><button (click)="delCol('inputs', $index)">✕</button></span> }
              <button class="add" (click)="addCol('inputs')">＋ input</button>
            </div>
            <div class="colset">
              <span class="lbl">Outputs</span>
              @for (f of outputs(); track $index) { <span class="chip out"><input [ngModel]="f.name" (ngModelChange)="rename('outputs', $index, $event)" /><button (click)="delCol('outputs', $index)">✕</button></span> }
          ...
          <div class="tablewrap">
            <table class="dt">
              <thead>
                <tr>
                  <th class="rn">#</th>
                  @for (f of inputs(); track $index) { <th class="in">{{ f.name || 'input' }}</th> }
                ...
                @for (r of rules(); track $index; let ri = $index) {
                  <tr>
                    ...
                    @for (f of inputs(); track f.name; let ci = $index) {
                      <td class="in">
                        <select [ngModel]="cellOp(ri, f.name)" (ngModelChange)="setCellOp(ri, f.name, $event)">@for (o of ops; track o) { <option [value]="o">{{ o }}</option> }</select>
                        @if (cellOp(ri, f.name) !== 'any') { <input [ngModel]="cellVal(ri, f.name)" (ngModelChange)="setCellVal(ri, f.name, $event)" placeholder="value" /> }
                      </td>
                    }
        ...
        <section class="card test">
          ...
          <div class="testrow">
            @for (f of inputs(); track f.name) {
              <label class="tin">{{ f.name }}<input [ngModel]="testInput()[f.name] || ''" (ngModelChange)="setTest(f.name, $event)" /></label>
            }
            <button class="btn" (click)="runTest()">Evaluate →</button>
          </div>
          @if (result(); as res) {
            @if (res.outputs) { <div class="res ok">matched rule(s) {{ resRuleLabels() }} → {{ json(res.outputs) }}</div> }
  ...
  styles: [`
    ...
    .res { margin-top:12px; padding:10px 12px; border-radius:9px; font-size:13px; font-family:ui-monospace,Menlo,monospace; } .res.ok { background:rgba(10,125,50,.1); color:#0a7d32; } .res.no { background:rgba(192,57,43,.1); color:#c0392b; }
  `],
})
export class DecisionDesignerComponent {
  ...
  private load(): void {
    this.caps.get(this.id()).subscribe((c) => {
      ...
      this.rules.set((b.rules ?? []).map((r) => ({ when: { ...r.when }, then: { ...r.then } })));
  ...
  private mutRules(fn: (r: DecisionRule[]) => void): void { this.rules.update((cur) => { const c = cur.map((r) => ({ when: { ...r.when }, then: { ...r.then } })); fn(c); return c; }); this.saved.set(false); }
