...
| **Permission scopes** — should the loyalty remote be allowed to register a tool that mutates payments? | Manifest declares scopes; runtime enforces | No scope enforcement | Add a `scopes` field to `RegistryEntry`; host policy gates registration |
...
| **Scopes** | `RegistryEntry.scopes?: readonly string[]`; host-side `RegistryPolicy` callback gates `register()` | Type extension + optional policy hook; default is no-op | ✓ no scopes = unrestricted as today |
