...
Twenty-two distinct scenarios the library covers, ranked roughly by adoption order — the first sixteen are in-app capabilities, the next five wire those same tools into the catalog platform and into external chat surfaces (Teams, GitHub Copilot, M365 Copilot Studio), and the final row covers post-chat surfaces (the agent escaping the chat rail). Pick the rows your team will hit; the rest are opt-in via DI.
...
| 22 | **Post-chat surfaces** — the agent reaches users outside the chat rail: persona-shaped workspace layouts, ⌘K palette, agent-computed table cells, per-row + bulk-selection menus, Cursor-pattern assist panel, proactive cron / webhook / queue triggers landing in a notification tray + inbox, user-built dashboards (tool / data / static tiles, conversational composition, live + drillable), workflow surfaces (multi-reviewer queue, investigation timeline, CAL training loop), versioned chain-hashed playbooks | `LayoutRegistry` · `<mvk-workspace-layout>` · `<mvk-cmd-k-palette>` · `<mvk-smart-cell>` · `<mvk-row-action-menu>` · `<mvk-bulk-toolbar>` · `<mvk-assist-panel>` · `TriggerRegistry` ([ADR-045](./docs/adr/0045-trigger-registry.md)) · `<mvk-notification-tray>` + `<mvk-inbox>` · `DashboardRegistry` ([ADR-044](./docs/adr/0044-dashboard-registry.md)) · `<mvk-dashboard-canvas>` + `TileResultCache` · `<mvk-review-queue>` · `<mvk-timeline-canvas>` · `<mvk-cal-workbench>` · `PlaybookRegistry` + `PlaybookRunner` + `<mvk-playbook-runner>` ([plan](./docs/plans/post-chat-surfaces-plan.md)) | Architects + product |
