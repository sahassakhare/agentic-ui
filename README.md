...
flowchart TB
    ...
    subgraph LIB["@infra-tools/agentic-ui (this library)"]
        ...
        L2["18 registries — Tool · Component · Action · Form · Approval · Operation · Trigger · Dashboard · Playbook · …"]:::lib
...
Twenty-seven distinct scenarios the library covers, ranked roughly by adoption order — the first twenty-two are in-app capabilities (sixteen foundational + six post-chat-surfaces pillars from the [P0–P5 plan](./docs/plans/post-chat-surfaces-plan.md)), the last five wire those same tools into the catalog platform and into external chat surfaces (Teams, GitHub Copilot, M365 Copilot Studio). Pick the rows your team will hit; the rest are opt-in via DI.
...
| 17 | **Persona-shaped workspace layouts (P0)** — promoted `LayoutRegistry` exposes slot-based layouts (rail / split / workspace / assist-panel / fullscreen); the LLM can emit a `LayoutDef` per turn or per route; per-persona policies pick the layout, ResizeObserver-driven slot resizing | `LayoutRegistry` · `<mvk-workspace-layout>` · `provideLayoutPolicy({...})` ([ADR-043](./docs/adr/0043-layout-registry-promotion.md), [cookbook](./docs/cookbook/agent-directed-workspace-layouts.md)) | Architects + product |
| 18 | **In-context agent affordances (P1)** — ⌘K command palette resolving intents → tools, agent-computed table cells with persona-scoped invocation, per-row context menus from `IntentRegistry`, selection-aware bulk toolbar, Cursor-pattern structured assist panel | `<mvk-cmd-k-palette>` · `<mvk-smart-cell>` · `<mvk-row-action-menu>` · `<mvk-bulk-toolbar>` · `<mvk-assist-panel>` ([cookbook](./docs/cookbook/cmd-k-palette.md)) | Architects + product |
| 19 | **Proactive triggers + inbox (P2)** — cron / webhook / queue triggers fire tools, actions, or notifications without a user prompt; landing zones are an unread-counted bell tray + a full inbox route + a multi-stage lifecycle widget for long-running flows | `TriggerRegistry` · `provideTriggerRunner({...})` · `<mvk-notification-tray>` · `<mvk-inbox>` · `<mvk-lifecycle-stages>` ([ADR-045](./docs/adr/0045-trigger-registry.md), [cookbook](./docs/cookbook/proactive-triggers-and-inbox.md)) | Architects + product |
| 20 | **User-built + conversational + live dashboards (P3)** — `DashboardDef` composes tiles from tool invocations / data-source queries / static props; LLM proposes layouts via `proposeDashboard`; cross-instance `TileResultCache` + refresh-on-event + drilldown; persona-blocked tiles render as no-access stubs, not 403s | `DashboardRegistry` · `<mvk-dashboard-canvas>` · `<mvk-dashboard-tile>` · `<mvk-dashboard-preview>` · `TileResultCache` ([ADR-044](./docs/adr/0044-dashboard-registry.md), [cookbook](./docs/cookbook/dashboards.md)) | Architects + product |
| 21 | **Workflow surfaces (P4)** — purpose-built widgets for multi-reviewer review queues (Workflow E), investigation timeline reconstruction (Workflow D), and Continuous Active Learning training loops (Workflow C); each composes existing Tool / Component / Approval registries with workflow-specific UI semantics | `<mvk-review-queue>` · `<mvk-timeline-canvas>` · `<mvk-cal-workbench>` ([cookbooks](./docs/cookbook/review-queue.md)) | Architects + product + compliance |
| 22 | **Versioned tool-call playbooks (P5)** — named, persona-scoped, federated tool chains the runtime fires sequentially with `origin: 'playbook'` chain-hashed audit; per-step `requiresApproval` halts on an approve/skip gate, `continueOnError` lets the run survive a failed step; signal-backed `RunningPlaybook` handle exposes live state + cancel | `PlaybookRegistry` · `PlaybookRunner` · `<mvk-playbook-runner>` ([cookbook](./docs/cookbook/playbooks.md)) | Architects + product + compliance |
| 23 | **Wire the catalog platform** — single composite provider for IAM persona + MFE registry + capability registrar/authorizer + usage metering | `provideAgenticPlatform({...})` ([ADR-031](./docs/adr/0031-provide-agentic-platform.md)) | Architects + execs |
| 24 | **External surface — Teams Tab embed** — same Angular app inside a Teams Tab, with tenant + UPN + theme bridged into the catalog | `provideTeamsContext({ loadContext })` ([cookbook](./docs/cookbook/teams-tab-embed.md)) | Architects + execs |
| 25 | **External surface — Teams chat (Bot Framework)** — converse with the agent in Teams chat; tool results render as Adaptive Cards | `@infra-tools/agentic-ui-teams-bot` ([cookbook](./docs/cookbook/teams-bot-adaptive-cards.md)) | Architects + execs |
| 26 | **External surface — GitHub Copilot Extension** — `@maverick-ediscovery` invocable from Copilot Chat in VS Code / JetBrains / github.com | `@infra-tools/agentic-ui-copilot-skill` ([cookbook](./docs/cookbook/github-copilot-extension.md)) | Architects + execs |
...
- **Post-chat surfaces (P0–P5).** The agent is reachable beyond the chat rail through 16 dispatch-agnostic widgets: `<mvk-workspace-layout>` (slot-based persona layouts via promoted `LayoutRegistry`, [ADR-043](./docs/adr/0043-layout-registry-promotion.md)), `<mvk-cmd-k-palette>` / `<mvk-smart-cell>` / `<mvk-row-action-menu>` / `<mvk-bulk-toolbar>` / `<mvk-assist-panel>` (in-context affordances), `<mvk-notification-tray>` + `<mvk-inbox>` + `<mvk-lifecycle-stages>` (proactive triggers — `TriggerRegistry`, [ADR-045](./docs/adr/0045-trigger-registry.md)), `<mvk-dashboard-canvas>` + `<mvk-dashboard-tile>` + `<mvk-dashboard-preview>` + `TileResultCache` (user-built + live + drillable dashboards — `DashboardRegistry`, [ADR-044](./docs/adr/0044-dashboard-registry.md)), `<mvk-review-queue>` / `<mvk-timeline-canvas>` / `<mvk-cal-workbench>` (workflow surfaces E / D / C), and `<mvk-playbook-runner>` + `PlaybookRunner` service (versioned, chain-hashed tool-call sequences via `PlaybookRegistry`). Every contribution federates through `defineCapabilityModule({ triggers, dashboards, playbooks, ... })`; `removeBySource` reaps them on unload. **Try the live demo** at [ediscovery-shell.onrender.com](https://ediscovery-shell.onrender.com) — the [guided tour](./docs/cookbook/post-chat-surfaces-tour.md) walks through every pillar with concrete click paths. See the [post-chat-surfaces plan](./docs/plans/post-chat-surfaces-plan.md) for the P0–P5 phase-by-phase rollout.
...
For the surface-by-surface enterprise-readiness analysis (what's wired vs prototype-only in the eDiscovery demo), see [`docs/analysis/2026-05-16-surface-by-surface-enterprise-readiness.md`](./docs/analysis/2026-05-16-surface-by-surface-enterprise-readiness.md).
...
- **`ApprovalRegistry`** + **`<mvk-approval-card>`** — HITL approval queue with async cross-session handoff (paralegal queues at 5pm, lead-counsel approves next morning). Per-approval audit + diff payload ([ADR-009](./docs/adr/0009-approval-intercept-and-audit-hook.md)).
...
- **`TriggerRegistry`** + **`provideTriggerRunner`** — browser-side cron triggers (`kind: 'cron'`); webhook + queue trigger shapes defined for server-side runners ([ADR-045](./docs/adr/0045-trigger-registry.md)).
...
- *`@infra-tools/agentic-ui-teams-bot`* — Microsoft Bot Framework adapter for Teams bot deployments ([§25 cookbook](./docs/cookbook/teams-bot-adaptive-cards.md)).
...
- *`@infra-tools/agentic-ui-copilot-skill`* — GitHub Copilot extension adapter ([§26 cookbook](./docs/cookbook/github-copilot-extension.md)).
- *`@infra-tools/agentic-ui-copilot-studio-connector`* — Power Platform Custom Connector adapter ([§27 cookbook](./docs/cookbook/copilot-studio-connector.md)).

---

**Reading the inventory.** Each tier above corresponds to one or more ADRs in [`docs/adr/`](./docs/adr/). The cookbook pages in [`docs/cookbook/`](./docs/cookbook/) walk through canonical wiring patterns; the eDiscovery flagship in [`examples/demo-ediscovery-shell`](./examples/demo-ediscovery-shell) exercises the bold-marked surface end-to-end.
...
![Eighteen registries grouped into Core / Extended / Seams, all inheriting one base class — image still depicts the original 13-registry layout; F4 Approval + F5 Operation + the three post-chat-surfaces additions (Trigger / Dashboard / Playbook) slot into the Extended tier](docs/assets/registry-tiers.png)
...
| [Registries vs. industry](./docs/architecture/registries-vs-industry.md) | Comparison of our 18 registries against agent SDKs (CopilotKit, LangChain, Vercel AI) and plugin platforms (VS Code, Backstage). Governance gaps + integration map onto the existing `RegistryBase`. |
...
| [Schematics reference](./docs/cookbook/schematics.md) | The 13 generators (`ng add`, `tool`, `widget`, `chat-shell`, `backend`, `agent-server`, `mfe-capability`, `action`, `intent`, `form`, `trigger`, `dashboard`, `playbook`) — all options + common pipelines. |
...
The npm-published packages cover **899 unit tests** across 74 spec files, executed in seconds via Vitest:

| Package | Tests | What's covered |
|---------|------:|----------------|
| `@infra-tools/agentic-ui` (runtime tier) | **811** | Registry base + 18 concrete registries · run orchestrator (turn lifecycle + tool execution + F4 approval + F5 long-running) · `defineCapabilityModule.apply()` / dispose (federation symmetry across all 18 registries) · cross-backend conformance suite · AG-UI converters + event mapper + RxJS bridge · MFE registry sources (static / Spring Boot / REST) · platform providers (catalog IAM, registrar, authorizer, usage metering, Teams context) · form / workflow / approval / operation-progress / dashboard / playbook-runner / cmd-k-palette / smart-cell / row-action-menu / bulk-toolbar / assist-panel / notification-tray / inbox / lifecycle-stages / review-queue / timeline-canvas / cal-workbench components · composition store + expression engine · `agentic-form` / `-widget` / `-workflow` / `-approval` factories · `PlaybookRunner` service · `TileResultCache` · schematics snapshot tests for all 13 generators |
