...
- **Schematics.** Fourteen generators: `ng-add`, `tool`, `widget`, `chat-shell`, `backend`, `agent-server`, `mfe-capability`, `connect-studio` (wire an app to an Experience Studio catalog backend via `provideCatalogRuntime` from `@infra-tools/agentic-ui/catalog` — all 13 governed kinds loaded at boot, live over one shared SSE connection; `--mode registries` embeds capabilities, `--shell` renders the whole catalog-driven app), `action`, `intent`, `form`, plus the post-chat-surfaces scaffolds `trigger`, `dashboard`, `playbook`. All snapshot-tested.
...
- **Observability.** `AgenticTelemetrySink` emit points are wired across the orchestrator (turn + tool-call lifecycle, approvals, LROs) and the registries — 70+ `agentic.*` events. Wire the zero-dep console sink with `provideAgenticTelemetryConsole()`, or an OpenTelemetry-backed sink via `provideAgenticTelemetry({ kind: 'otel', providers })` (see `docs/cookbook/observability.md`). W3C trace-context propagation across SSE is a planned follow-up (ADR-001), not yet shipped.
