...
|---|---|---|
| 18 registries on a uniform `Registry<TDef>` shape | `projects/agentic-ui/src/lib/registries/` | Tool, Component, Capability, Backend, MFE + Action, Intent, Form, DataSource + Validation, Persistence, Layout, SchemaTransformer + Approval (F4), Operation (F5), Trigger ([ADR-045](./docs/adr/0045-trigger-registry.md)), Dashboard ([ADR-044](./docs/adr/0044-dashboard-registry.md)), Playbook (post-chat-surfaces P5) |
...
| **Enterprise reference application** | `examples/demo-ediscovery-{shared,server,shell,review,production,search,mcp}/` | Phases 0–8 of the [eDiscovery plan](./docs/plans/ediscovery-app-plan.md) PLUS Capabilities F1–F6 from the [r3 plan](./docs/plans/ediscovery-dynamic-ui-plan.md): 25+ tools across 4 specialists, 3 federated MFE remotes, all 18 registries exercised (including F4 `ApprovalRegistry` + F5 `OperationRegistry` + post-chat-surfaces `TriggerRegistry` / `DashboardRegistry` / `PlaybookRegistry`), tamper-evident chain-of-custody extended with `tool-approved` / `tool-rejected` / `operation-*` / `trigger-*` / `playbook-*` events, MCP for analyst desktops, persona permission filter, `/approvals` queue + `/operations` panel routes. |
...
| **Post-chat surfaces program (P0–P5)** | `projects/agentic-ui/src/lib/{layout,registries,components}/` + [plan](./docs/plans/post-chat-surfaces-plan.md) | The agent escaped the chat rail across six phases. **P0**: promoted `LayoutRegistry` ([ADR-043](./docs/adr/0043-layout-registry-promotion.md)) + `<mvk-workspace-layout>` + `provideLayoutPolicy({...})` (rail / split / workspace / assist-panel / fullscreen modes). **P1**: five dispatch-agnostic in-context affordances — `<mvk-cmd-k-palette>`, `<mvk-smart-cell>`, `<mvk-row-action-menu>`, `<mvk-bulk-toolbar>`, `<mvk-assist-panel>`. **P2**: `TriggerRegistry` ([ADR-045](./docs/adr/0045-trigger-registry.md)) + `provideTriggerRunner({...})` + `<mvk-notification-tray>` + `<mvk-inbox>` + `<mvk-lifecycle-stages>` (cron / webhook / queue triggers → tool / action / notification dispatch). **P3**: `DashboardRegistry` ([ADR-044](./docs/adr/0044-dashboard-registry.md)) + `<mvk-dashboard-canvas>` + `<mvk-dashboard-preview>` + `TileResultCache` (user-built + conversational + live + drillable dashboards with discriminated `TileInvocation`). **P4**: `<mvk-review-queue>` (Workflow E) + `<mvk-timeline-canvas>` (Workflow D) + `<mvk-cal-workbench>` (Workflow C) workflow surfaces. **P5**: `PlaybookRegistry` + `PlaybookRunner` service + `<mvk-playbook-runner>` (versioned, persona-scoped, federated tool-call sequences with `requiresApproval` + `continueOnError` step modifiers, chain-hashed `origin: 'playbook'` audit). **Post-P5**: `defineCapabilityModule` extended to federate all three new registries symmetrically with `removeBySource` reap on unload + three new schematics generators (`trigger` / `dashboard` / `playbook`). 16 widgets + 3 ADRs + 16 cookbooks + +352 lib specs across the program. |
