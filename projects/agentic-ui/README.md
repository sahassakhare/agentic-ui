...
- **Studio catalog consumption.** The `@infra-tools/agentic-ui/catalog` secondary entry turns governed, Experience-Studio-authored capabilities into live registry entries with one call — `provideCatalogRuntime({ baseUrl, tenant }, { mode: 'registries' \| 'shell' })` — re-hydrated live over one shared SSE connection. `registries` mode embeds capabilities in an existing app; `shell` mode renders the whole catalog-driven application. Kept off the primary FESM ([ADR-052](https://github.com/sahassakhare/agentic-ui/blob/main/docs/adr/0052-catalog-consumption-secondary-entry.md)); scaffold it with `ng g @infra-tools/agentic-ui:connect-studio`.
