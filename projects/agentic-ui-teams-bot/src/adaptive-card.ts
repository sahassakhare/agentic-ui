 ...
 * activity.
 *
 * Tools that emit an `adaptiveCard` render hint pass through
 * untouched. Tools that don't fall through to the generic builder
 * which converts a `components: [{name, props}]` array into a
 * "what just happened" fact set + an "Open in app" deep-link
 * action.
 *
...
/** Wrap a raw card JSON into a Bot Framework attachment. */
...
/** Build a hello card -- used for `conversationUpdate` (bot
 *  added to a chat). Adopters can override with their own when
 *  they want branding. */
  ...
  return {
    ...
    body: [
      ...
      },
      {
        ...
        text:
          opts?.body
          ?? 'Ask me to onboard a custodian, place a legal hold, or ' +
             'show productions. I\'ll use the same tools your web app does.',
        wrap: true,
      },
    ],
  };
 ...
 * explicit `adaptiveCard` hint -- we still want SOMETHING to show
 * up in Teams, even if it's spartan.
 *
 * Pattern:
 *   - Card title = widget name (or props.title if set).
 *   - Body = a FactSet of every primitive prop (string, number,
 *     boolean). Object / array props are stringified at one
 *     level deep.
 *   - Optional `Action.OpenUrl` button when the host passes
 *     `deepLinkUrl` -- lets operators jump from the chat card
 *     into the Teams Tab for the full Angular surface.
 *
 * Hand-AC'd schemas always beat this fallback when the tool
 * supplies one via the `adaptiveCard` render hint.
 */
...
function collectFacts(props: unknown): Array<{ title: string; value: string }> {
  ...
  for (const [key, raw] of Object.entries(props as Record<string, unknown>)) {
    if (['name', 'title', 'label'].includes(key)) continue;   // shown in title slot
    const value = scalarize(raw);
    if (!value) continue;
    out.push({ title: humanize(key), value });
    if (out.length >= 8) break;   // keep cards compact
  }
  return out;
}
